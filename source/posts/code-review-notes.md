...
tags: [code review, dart, flutter, spread operator]
---
...
так это совершать ошибки. И один из способов научиться не совершать ошибки, это учиться на
...
код-ревью должным образом. Отсюда и возникает ситуация, когда разработчик имеет некоторый опыт
 (порой даже в нескольких компаниях), имеет солидное резюме, но по факту не умеет писать
...
объяснение проблемы в комментариях к MR-у может быть и не так напряжено. Но писать одно и тоже
десятками раз довольно быстро утомляет. А это довольно сильно сказывается на качестве код-ревью.
А мы, все таки, здесь пытаемся говорить о качественном код-ревью. 

![image](./step_on_a_rake.png)

Люди так устроенны, что несмотря на многократные попытки разъяснить какие-то моменты, обсуждения и
комментарии к MR-у, это ровным счетом никак не гарантируют, что тот же самый разработчик не
совершит эту же самую ошибку, не далее чем завтра, в соседнем MR-е. Таким образом написание подобных
...
нет возможности быть хорошо проревьювенным.
...
Это обычный Flutter код с разметкой некоторого виджета. С подобным кодом
Flutter разработчики встречаются каждый божий день. На первый взгляд может даже показаться,
...
код нельзя пропускать далее MR-а. Это как раз одна из причин почему делать код-ревью нужно и делать
...
После того как мы "смысли" весь синтаксический сахар стало еще более наглядно видно в чем же
...
Очевидно, что это совсем не то, что автор MR-а хотел реализовать. Как и всегда дьявол кроется
...
Таким образом используемая по умолчанию реализация множества использует `LinkedHashSet`, который,
...
работать. Например, команда Dart\`а может решить, что реализация множества недостаточно быстрая и
заменить на какую-то другую реализацию. В таком случае как раз очень велика вероятность того,
что подобный код сломается. И такое вполне может случиться. Учитывая то, что `Set` в своем контракте
указывает, что порядок не задан, у разработчиков Dart\`а развязаны руки. И такое вполне может произойти.
Так же стоит помнить, что флаттер проекты собираются под множество платформ - мобильные, десктоп,
...
усложняет код. И давайте представим, что такой и ему подобный код разросся по проекту. Это будет
довольно мучительно поддерживать такой код.
...
`if () ...{}`. У автора кода могло сложиться впечатление, на интуитивном уровне, что эти два куска
кода идентичны, хотя это и не так.

## Двусмысленность синтаксиса Dart`а

Язык Dart поддерживает довольно удобный синтаксис объявления коллекций. Который называется
List/Set/Map comprehension. Не знаю как этот термин лучше перевести на русский язык. Благодаря
...
Collection comprehension синтаксис позволяет создавать и инициализировать разные коллекции
...
то может показаться, что код идентичен. Но это не так. Если посмотреть на то, что будет распечатано,
...
Мы получаем два списка, но с разными типами. В первом случае будет `List<Object>`, а во втором
`List<Text>`.
...
В нашем же случае элементы списка имеют тип `Object`, и такой код не скомпилируются. Второй элемент
нашего списка это множество состоящее из одного виджета. Как уже выше было отмечено, из-за того,
что `Set` реализует `Iterable<T>` и List Comprehension синтаксису в купе с Spread operator`ом код,
рассматриваемый в нашей статье и получился. Еще раз обратите внимание на следующий момент:
...
Здесь фигурные скобки создают множество, а не область видимости (Scope) для if statement. Как раз
из-за этой двусмысленности в языке, а как следствие и возможности ошибиться, и возникла эта статья.
...
разобраться без сторонней помощи может быть не просто. 
