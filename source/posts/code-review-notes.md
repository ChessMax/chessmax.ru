...
объяснение проблемы в комментариях к _MR_'у может быть и не так напряжено. Но писать одно и тоже
...
комментарии к _MR_'у, это ровным счетом никак не гарантируют, что тот же самый разработчик не
совершит эту же самую ошибку, не далее чем завтра, в соседнем _MR_'е. Таким образом написание подобных
...
что `Set` реализует `Iterable<T>` и _List Comprehension_ синтаксису в купе с _Spread operator_`ом код,
